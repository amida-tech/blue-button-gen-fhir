...
exports.observationForLabResult = function (result, bundle) {
    if (result.name && result.related && result.related.length) {
        ...
        if (n > 1) {
            ...
            if (targetId && bundle && bundle.count()) {
                for (var i = bundle.count() - 1; i >= 0; --i) {
                    ...
                    var id = bundle.id(i);
                    if (id === targetId) {
                        var relatedCode = bbuo.deepValue(bundleResource, 'name.coding.0.code');
                        var code = bbuo.deepValue(result, 'name.coding.0.code');
                        if (relatedCode === code) {
                            return null;
                        }
                    }
