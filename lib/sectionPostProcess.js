...
module.exports.vitals = (function () {
        ...
        if (panelInfo) {
            var panelTemplate = {
                content: {
                    ...
                    issued: {
                        dataKey: 'bundleEntries.0.content.issued'
                    },
                    status: 'final',
                    reliability: 'ok',
                    subject: {
                        dataKey: 'bundleEntries.0.content.subject'
                    },
                    extension: {
                        constant: [{
    ...
    var getDate = function (bundleEntry) {
        return moment(bundleEntry.content.issued).format('YYYYMMDD');
    };

    var getPanelCode = function (bundleEntry) {
        var code = bundleEntry.content.name.coding[0].code;
