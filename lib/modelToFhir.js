...
var entryToBundle = function (sectionName, entry) {
    log.trace({
        entry: entry
...
var demographicsToBundle = function (demographics) {
...
var modelToFHIR = exports.modelToFHIR = function (model, patientEntry, externalize) {
    ...
    if (patientEntry) {
        var name = patientEntry.content && patientEntry.content.name && patientEntry.content.name[0];
        var family = (name && name.family) || [];
        var given = (name && name.given) || [];
        var patient = {
            reference: patientEntry.id,
