...
var bundlePrototype = {
    ...
    addBundle: function (bundle) {
        arrayset.append(this.entry, bundle.entry);
    },
    addEntry: function (entry) {
        this.entry.push(entry);
    },
    toArray: function () {
        var result = this.entry.map(function (entry) {
    ...
    id: function (index) {
        return this.entry[index].id;
    },
    resource: function (index) {
        return this.entry[index].content;
    },
    count: function () {
    ...
    updatePatient: (function () {
        ...
        return function (patientReference) {
            this.entry.forEach(function (entry) {
                ...
                if (content) {
                    var type = content.resourceType;
                    var property = propertyPerResourceType[type];
                    if (property) {
                        content[property] = patientReference;
                    }
                }
            });
        };
    ...
    toResource: function () {
        return {
            resourceType: "Bundle",
            totalResults: this.entry.length,
            entry: this.entry
        };
    }
};
